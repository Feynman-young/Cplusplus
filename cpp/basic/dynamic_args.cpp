...
template <typename T, typename... Types> void print(T first_arg, Types... args)
{
...
template <typename T> Node<T> *Node<T>::*left = &Node<T>::left;

template <typename T> Node<T> *Node<T>::*right = &Node<T>::right;

template <typename T, typename... TP> Node<T> *traverse(Node<T> *np, TP Node<T>::*...paths)
{
    return (np->*...->*paths); // np ->* paths1 ->* paths2 ...
}

template <typename T> class SpacePrint
{
  private:
    T const &ref;
...
template <typename... T> void printWithSpace(T... args)
{
...
template <typename C, typename... Index> void printElements(C const &c, Index... index)
{
    print(c[index]...);
}

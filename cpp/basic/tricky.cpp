...
template <typename T> void printContainer(T const &c)
{
...
template <typename T> struct CArray; // primary template

template <typename T, std::size_t SZ> struct CArray<T[SZ]>
...
template <typename T> class Stack
{
  ...
  public:
    ...
    template <typename V> Stack &operator=(Stack<V> const &);
};
...
template <typename T> template <typename V> Stack<T> &Stack<T>::operator=(Stack<V> const &s)
{
    Stack<V> tmp(s);
    elements.clear();
    while (!tmp.empty())
...
template <typename T = long double> constexpr T PI{3.14159265358};

...
template <typename T> void printByValue(T arg)
{
...
template <typename T> void printByReference(T &arg)
{
    std::cout << typeid(T).name() << ": " << arg << "\n";
}
...
template <typename T> T retR(T &&p)
{               // p is a forwarding reference
    return T{}; // OOPS: returns by reference when called for lvalue;
}

template <typename T> T retV(T p)
{               // p might be a reference
    return T{}; // OOPS: returns a reference if T is a reference
}

// For more safe, there are two options
// 1. Convert type T to a nonrefence
template <typename T> typename std::remove_reference<T>::type retVConvert(T p)
{
    return T{};
}

// 2. Use auto
template <typename T> auto retVAuto(T p)
{
    return T{};
}

template <typename T1, typename T2>
constexpr std::pair<typename std::decay<T1>::type, typename std::decay<T2>::type> make_pair(T1 &&a, T2 &&b)
{
    return std::pair<typename std::decay<T1>::type, typename std::decay<T2>::type>(std::forward<T1>(a),
                                                                                   std::forward<T2>(b));
}

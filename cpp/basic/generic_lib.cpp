...
template<typename T>
void printAddress(T&& x) {
	auto p = std::addressof(x);
	std::cout << p << '\n';
}

template<typename T>
void paramIsReference(T) {
...
public:
	...
	std::conditional<std::is_move_constructible_v<D>, D&&, D&>::type foo() {
...
int main() {
	...
	foreach(n.cbegin(), n.cend(), [] (int i) {
		std::cout << "Lambda for: " << i << '\n';
		});

	Func f;
	foreach(n.cbegin(), n.cend(), &Func::member_func, f);
	foreach(n.cbegin(), n.cend(), [] (std::string const& s, int i) {
		std::cout << s << i << '\n';
		}, "value: ");

...
class PersonT
{
  ...
  public:
    ...
    template <typename STR> explicit PersonT(STR &&s) : name(std::forward<STR>(s))
    {
...
template <typename T> using EnableIfString = std::enable_if_t<std::is_convertible_v<T, std::string>>;
...
class PersonE
{
  ...
  public:
    // Generic constructor for passed initial name;
    template <typename STR, typename = EnableIfString<STR>> explicit PersonE(STR &&s) : name(std::forward<STR>(s))
    {
        std::cout << "TMPL-CONSTR for " << name << "\n";
    }
